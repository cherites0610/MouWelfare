...
export class DataProcessingService extends WorkerHost {
    ...
    async process(job: Job<CrawlData>): Promise<void> {
        ...
        try {
            ...
            await this.welfareService.create({
                ...
                categoryID: processedData.category.map((item) => this.constService.getCategoryIDByName(item)),
                identityID: processedData.identity.map((item) => this.constService.getIdentityIDByName(item)),
            })
    ...
    private async category(content: string): Promise<string[]> {
        ...
        const temp = ["兒童及青少年福利", "婦女與幼兒福利", "老人福利", "社會救助福利", "身心障礙福利"]
        if (response.text) {
    ...
    private async identity(content: string): Promise<string[]> {
        ...
        const temp = ["20歲以下", "20歲-65歲", "65歲以上", "男性", "女性", "中低收入戶", "低收入戶", "榮民", "身心障礙者", "原住民", "外籍配偶家庭"];

        if (response.text) {
            const identity = (response.text as string)
                .split('|')
                .map(c => c.replace(/\s+/g, '').replace(/\n/g, '').replace(/\r/g, '').trim()) // 清理字串
    ...
    async retryWithDelay<T>(
        ...
        while (attempt <= maxRetries) {
            try {
                ...
                this.logger.warn(`功能失敗，${delayMs / 1000} 秒後重試：${err.message}`);
