...
export class FamilyService {
  ...
  async updateName(userID: string, familyID: string, name: string) {
    ...
    const userFmaily = family.userFamilies.find(
    ...
    if (
      !(
        userFmaily.role == FamilyRole.Admin ||
        userFmaily.role == FamilyRole.Creator
      )
    ) {
  ...
  async updateRole(
    ...
    const setterUserFmaily = family.userFamilies.find(
      (item) => item.user.id === setterUserID,
    );
    if (!setterUserFmaily) {
      throw new NotFoundException("家庭中沒有該用戶");
    }
    ...
    const targetUserFmaily = family.userFamilies.find(
    ...
    await this.userFamilyService.updata(
      targetUserFmaily.id,
      targetUserID,
      familyID,
      role,
  ...
  async joinFamilyByInviteCode(userID: string, code: string): Promise<Family> {
    ...
    // 檢查用戶是否已在家庭中
    const existingUserFamily = family.userFamilies.find(
      (item) => item.user.id === userID,
    );
