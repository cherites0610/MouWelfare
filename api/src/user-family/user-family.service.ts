...
export class UserFamilyService {
  ...
  async create(createUserFamilyDto: CreateUserFamilyDto) {
    const userFamily = this.userFamilyRepository.create({ role: createUserFamilyDto.role, family: { id: createUserFamilyDto.familyID }, user: { id: createUserFamilyDto.userID } })
    await this.userFamilyRepository.save(userFamily)
    return userFamily;
  }

  async findOneByUserID(userID: string) {
    const userFamily = await this.userFamilyRepository.findOneBy({ user: { id: userID } })
    if (!userFamily) {
  ...
  async updata(id: string, userID: string, familyID: string, role: FamilyRole) {
    const userFamily = this.userFamilyRepository.create({id:id,user:{id:userID},family:{id:familyID},role:role})
    await this.userFamilyRepository.save(userFamily)
