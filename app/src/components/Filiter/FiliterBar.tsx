...
import {
  ...
  LocationNum,
  ServiceNum,
} from "@/src/utils/getTextByNumber";
...
const FilterBar: React.FC<FilterBarProps> = React.memo(
  ({ openFilterDrawer }) => {
    ...
    const { locations, categories, family, identities, age, gender, income } =
    ...
    const LOCATION: string[] = Array.from({ length: LocationNum - 1 }, (_, i) =>
      getTextByLocation(i + 1)
    );
    const CATEGORY: string[] = Array.from({ length: ServiceNum - 1 }, (_, i) =>
      getTextByService(i + 1)
    );

    // Toggle modal visibility
    const toggleModal = useCallback(
      (key: "location" | "category" | "family", visible: boolean) => {
        switch (key) {
          case "location":
    ...
    const detailedFilterCount =
      identities.length + income.length + (age ? 1 : 0) + (gender ? 1 : 0);
    ...
    return (
      <View style={styles.filterBar}>
        ...
        <FilterButton
          selectedCount={detailedFilterCount}
          label="篩選"
