  ...
  const RightAction = ({
    ...
    const handleFavorite = async () => {
      try {
        if (item.isFavorited) {
          // 如果已收藏，則呼叫刪除 API
          deleteFavoriteAPI(authToken, item.id);
        } else {
          // 如果未收藏，則呼叫新增 API
          addFavoriteAPI(authToken, item.id);
        }
        // 呼叫父層傳來的函式，來更新 UI 狀態
      ...
      <Reanimated.View style={[styleAnimation, { flexDirection: "row" }]}>
        ...
        >
          ...
          <Ionicons
            ...
            size={28}
            color={item.isFavorited ? "white" : "black"}
          />
        </TouchableOpacity>
        <TouchableOpacity
          style={{
          ...
          <TouchableOpacity
            onPress={() => {
              const lightReasonString = item.lightReason
                ...
                : "";
              route.navigate(
                `/home/${item.id}?lightStatus=${item.lightStatus ?? -1}&lightReason=${lightReasonString}`
              );
            }}
          >
            <WelfareItem
