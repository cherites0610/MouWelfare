...
import {
  ...
  setIncome,
  resetFilters,
} from "@/src/store/slices/filiterSlice";
import {
  ageOptions,
  genderOptions,
  incomeOptions,
  identityOptions,
} from "@/src/components/FliterDrawer/constants";
...
export default function Index() {
  ...
  const { autoFilterUserData, authToken } = useSelector(
    (state: RootState) => state.config
  );
  const dispatch = useDispatch<AppDispatch>();

  const fetchWelfareData = useCallback(
    (queryParams: Partial<WelfareApiParams>, isNextPage = false) => {
      ...
      fetchWelfareApi(query)
        .then(async (res) => {
          ...
          const favoriteIds = new Set(
            favoritesResponse.data.map((fav: Welfare) => fav.id)
          );
          ...
          const sortedData = enrichedData.sort((a, b) => {
            ...
            if (locationMatchA !== locationMatchB)
              return locationMatchA - locationMatchB;

            // 3️⃣ 年齡匹配優先
            const ageMatch = (card: Welfare) => {
              if (!age) return 1;
              return card.applicationCriteria.some((c) => c.includes(age))
                ? 0
                : 1;
            };
            const aa = ageMatch(a);
          ...
          setHasMore(
            res.data.pagination.totalPage - res.data.pagination.page > 0
          );
        })
        .catch((err) => {
  ...
  const handleRefresh = useCallback(() => {
    setPage(1);
    fetchWelfareData(
      { locations, categories, identities, searchQuery, age, gender, income },
      false
    );
  }, [
    fetchWelfareData,
  ...
  const debouncedHandleRefresh = useCallback(debounce(handleRefresh, 300), [
    handleRefresh,
  ]);
  ...
  useEffect(() => {
    ...
    if (autoFilterUserData) {
      ...
      if (user.identities && user.identities.length > 0) {
        ...
        // 篩選出收入 (多選)
        const foundIncome = incomeOptions.filter((opt) =>
          userIdentities.includes(opt)
        );
        dispatch(setIncome(foundIncome));

        // 篩選出身分別 (多選)
        const foundIdentities = identityOptions.filter((opt) =>
          userIdentities.includes(opt)
        );
  ...
  const handleToggleFavoriteInList = useCallback(
    (welfareId: number, currentStatus: boolean) => {
      setData((currentData) =>
  ...
  return (
    <GestureHandlerRootView>
      ...
      >
        ...
        <Search />
        <Filiter openFilterDrawer={openDrawer} />
        <ReanimatedDrawerLayout
          ref={drawerRef}
          renderNavigationView={() => (
            <FilterDrawer closeDrawer={closeDrawer} />
          )}
